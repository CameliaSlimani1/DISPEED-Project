...
structure = {
    'layers': [
        ...
        {'type': 'conv2d','params': {'filters': 128 , 'kernel_size' : (3,1), 'activation': "relu",'padding':'same'}},
        {'type': 'conv2d','params': {'filters': 128 , 'kernel_size' : (3,1), 'activation': "relu",'padding':'same'}},
        {'type': 'conv2d','params': {'filters': 128 , 'kernel_size' : (3,1), 'activation': "relu",'padding':'same'}},
        {'type': 'maxpool2d','params': {'pool_size': (2,1)}},
        {'type': 'conv2d','params': {'filters': 256 , 'kernel_size' : (3,1), 'activation': "relu",'padding':'same'}},
        {'type': 'conv2d','params': {'filters': 256 , 'kernel_size' : (3,1), 'activation': "relu",'padding':'same'}},
        {'type': 'conv2d','params': {'filters': 256 , 'kernel_size' : (3,1), 'activation': "relu",'padding':'same'}},
        {'type': 'maxpool12','params': {'pool_size': (2,1)}},
        {'type': 'flatten', 'params': {}},
...
cnn = IDSModel ("NoFS-CNN", "CNN", structure)
x_train = x_train.reshape(x_train.shape[0], x_train.shape[1], 1, 1)

x_test = x_test.reshape(x_test.shape[0], x_test.shape[1], 1, 1)
cnn.create_model(x_train, y_train, x_test, y_test)
cnn.generate_tflite_model(x_test, False)"""
#cnn.load_ids_model("../../output/models/CNN/ES-CNN.h5")
#rf_ids.explore_rf_models(x_train, y_train, x_test, y_test, None, None)
"""es_cnn = IDSModel("ES-CNN", "CNN", None)
es_cnn.load_ids_model("../../output/models/CNN/NoFS-CNN.h5")
...
"""
read_and_plot_energy_from_file("../../output/energy_measures/dnn1_es_fpga.csv")
